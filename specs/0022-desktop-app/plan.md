...
electron-builder.yml     Packaging → NSIS Windows installer. Pins the icon, product metadata, a
                         space-free artifact name, and an EXPLICIT signing posture
                         (`signExecutable: false` for the alpha — never
                         `signAndEditExecutable: false`, which also drops the icon/metadata
                         resource-edit pass while still exiting 0).
build-resources/icon.ico App + installer icon, GENERATED by scripts/generate-icon.mjs. Not `build/`:
                         that path is git-ignored here as a compiler output dir, so an icon there
                         would be absent from a clean checkout and CI, and electron-builder would
                         silently fall back to the default Electron icon.
scripts/generate-icon.mjs      Deterministic, dependency-free .ico generator (stdlib only).
scripts/checksum-release.mjs   Writes/verifies release/SHA256SUMS.txt — the only integrity signal
                               an unsigned alpha installer has.
docs/RELEASE.md          Build, signing posture, checksum verification, manual release checklist.
```
