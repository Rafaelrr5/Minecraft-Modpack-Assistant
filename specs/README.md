...
| [0022](./0022-desktop-app/spec.md) | Friendly Desktop App — an **Electron** GUI over the full lifecycle as a second adapter (main runs the UI-agnostic core in-process; isolated React renderer); dry-run→confirm safety surfaced on screen; Electron-free composition root covered by `npm run check`, Electron shell built by electron-vite (amends ADR 0003 via ADR 0008) | Desktop | in-progress |
| [0023](./0023-distribution-gate/spec.md) | Distribution Gate — `unresolved`/`unsatisfied-dependency`/`incompatible` resolution issues **block** build, export and release (no instance, no archive, no plan to confirm, distinct exit code); one explicit `--allow-unsupported` expert override stamps the artifact `MPA-UNSUPPORTED.txt` and renames archives `-unsupported`; a `provider-error` stays *unknown* and only warns | 7 | done |
| [0024](./0024-export-overrides/spec.md) | Export Overrides — ship the pack's real non-mod content (configs, KubeJS, FTB Quests, resource/shader packs) inside `overrides/`, collected read-only through the guarded `InstanceFs` under a **deny-by-default whitelist** (worlds, logs, backups, `mods/` and credential files can never travel), carried **byte-for-byte** and proven by archive parse-back; an artifact with no overrides is declared **mods-only** | 7 | done |
| [0025](./0025-launchable-handoff/spec.md) | Launchable Handoff — project a pinned pack into a **Prism Launcher** instance (`mmc-pack.json` + `instance.cfg`, pinned loader build and `-Xmx`) or **Modrinth App** `.mrpack` import, verified against the launcher's own metadata feed (an unreachable feed stays *unknown*, never a false "missing"); closes the gap ADR 0007 left — the assistant never bootstraps a Minecraft client, the launcher does | 7 | done |
| [0026](./0026-open-source-release-package/spec.md) | Open-Source Opening Package — licence, contributor guide, security policy + threat model, support policy, changelog, GitHub issue/PR templates, and a README maturity table that separates the stable CLI from the alpha desktop app and from `launch`'s documented limits; plus a credential scan of the whole git history. Documentation only — no runtime change | 8 | done |
